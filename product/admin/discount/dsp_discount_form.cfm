...
<cfswitch expression="#discount#">
	...
	<cfcase value="edit">
		...
		<cfloop list="#fieldlist#" index="counter">
			<cfset attributes[counter] = qry_get_discount[counter][1]>
		</cfloop>
...
<cfhtmlhead text="
	...
	<script type='text/javascript' src='#Request.StorePath#includes/openwin.js'></script>
">
...
<cfif attributes.discount is "add">
	...
	<input type="radio" name="Type3" value="4"/> Order <span class="formtextsmall">(applies to the order total, after taxes applied)</span>
<cfelse>
	...
	<!--- Display --->
		<tr>
			...
			<td style="background-color: ###Request.GetColors.formreq#;"></td>
		 	<td>
			<input type="text" name="Disc_Display" value="#attributes.Display#" size="40" maxlength="255" class="formfield"/>
			</td></tr>			
	...
	<!--- Coupon Code --->
		<tr>
			...
			<br/><span class="formtextsmall">Customer must enter this code to receive the discount. Codes are case insensitive.</span>
			</td></tr>	
...
objForm.required("Name,Disc_Display,MinOrder,OneTime");
...
objForm.Disc_Display.description = "display to users";
