...
<cfif isDefined("attributes.submit_import")>
	...
	<cfscript>
		...
		if (len(Request.TempPath) ) {
			theDirectory = Request.TempPath;
		} else {
			theDirectory = GetDirectoryFromPath(ExpandPath(Request.StorePath));
	...
	<cftry>
		...
		<cffile action="UPLOAD" filefield="importcsv" destination="#theDirectory#" nameconflict="MAKEUNIQUE">
		
		<!--- check file extension --->
		<cfif File.ServerFileExt IS "csv" AND NOT Find(File.ServerFile, ";")>
			<!--- read in the csv file --->
			<cffile action="READ" file="#theDirectory##File.ServerFile#" variable="importdata">	
		
		<cfelse>
			<cfset attributes.error_message = "There was an error uploading the file to the server. Please only upload .csv (comma-delimited) files." />
		</cfif>
