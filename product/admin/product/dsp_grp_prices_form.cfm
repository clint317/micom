...
<cfif isdefined("attributes.edit")>
	...
	<cfinclude template="qry_get_grp_price.cfm">

	<cfloop list="gid,price,grpprice_id" index="counter">
		<cfset attributes[counter] = GetPrice[counter][1]>
	</cfloop>
...
<!--- Add form ---->
	<form name="editform" action="#self#?fuseaction=product.admin&do=Grp_Price#request.token2#" method="post">
...
<cfoutput>
		...
		<img src="#Request.ImagePath#spacer.gif" alt="" width="1" height="1" /></td></tr>
		<tr>
			<th>User Group</th>
