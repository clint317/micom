...
<input type="text" name="Freight_Intl"  class="formfield" value="#iif(attributes.Freight_Intl IS NOT 0, NumberFormat(attributes.Freight_Intl, '0.00'), DE(''))#" size="7" maxlength="15"/> #Request.AppSettings.MoneyUnit# (International) 
		...
		<tr>
			...
			<span class="formtextsmall">For oversize items that ship separately, enter all dimensions or none<br/>Used for automated shipping methods only</span> </td></tr>
...
<tr>
	...
	<!--- Content ---->
		<tr>
			...
			<td style="background-color: ###Request.GetColors.formreq#;"></td>
		 	<td>
			<input type="text" name="content_url" value="#attributes.content_url#" size="40" maxlength="100" class="formfield"/>  <a href="JavaScript: newWindow = openWin('#self#?fuseaction=home.admin&select=image&type=Download&fieldname=content_url&fieldvalue=#attributes.content_url#&user=#attributes.user_id#', 'images', 'width=850,height=700,toolbar=0,location=0,directories=0,status=1,menuBar=0,scrollBars=0,resizable=1'); newWindow.focus()" class="formtextsmall">File Manager</a>
			</td>
		</tr> 
...
<cfquery name="member_prods" datasource="#Request.ds#" username="#Request.DSuser#" password="#Request.DSpass#" >
	...
	AND Product_ID <> #attributes.product_id#
			...
			<tr>
				...
				<span class="formtextsmall"><br/>For use with Recurring Products only. Use this field for a low-priced trial membership that can only be purchased once. When this membership expires, it will be renewed using the product listed here. Leave blank for memberships that just auto-renew themselves. <br/>NOTE: This feature is only supported for payment gateways that use tokens (Shift4).</span>
				</td>
			</tr>		
