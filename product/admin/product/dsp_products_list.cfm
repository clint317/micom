...
<cfset fieldlist="do,name,sku,display_status,highlight,potd,sale,hot,type,event_id,account_id,cid,nocat">
...
<cfset session.formKeys.productList = formkey>	
...
<cfif qry_get_products.recordcount gt 0>					
	...
	<cfloop query="qry_get_products" startrow="#PT_StartRow#" endrow="#PT_EndRow#">
		...
		<td <!--- <cfif qry_get_products.catname is "">bgcolor="yellow"</cfif> --->><a href="#Request.StoreSelf#?fuseaction=product.display&product_id=#product_id##Request.Token2#" target="store" #doMouseover('View Product in Store')#>#name#</a></td>		
		...
		<td><a href="#self#?fuseaction=product.admin&do=copy&dup=#product_id#<cfif len(attributes.cid)>&cid=#attributes.cid#</cfif>&formkey=#Hash(formkey,"SHA-256")##Request.Token2#" #doMouseover('Copy Product')#>copy</a></td>
	</tr>
...
<cfoutput><script type="text/javascript" src="#Request.StorePath#includes/tooltips/wz_tooltip.js"></script></cfoutput>
