...
<!--- Now set the Attribute scoped variables to locals, so that our code appears a little neater to the eye. --->
	...
	<cfset formkey=			 Attributes.formkey>
...
<!--- Query the database to see if we have an image to display. --->
	<cfquery name="GetRecordImages" datasource="#Request.DS#" username="#Request.DSuser#" password="#Request.DSpass#">
...
<!--- Construct the image name so that we may refer to it as a variable. --->
	<cfset TheImage=GetRecordImages[TheFileColumnName][1]>
...
<!--- [2] Form layout table --->
	<cfoutput><table class="formtext" style="color: ###request.getcolors.InputTText#;"></cfoutput>
		...
		<cfelse>
			<td>
			<cfoutput><img src="#Request.ImagePath#spacer.gif" alt="" width="56" height="1" /></cfoutput>
			</td>
		</cfif>
...
<!--- [4] Process if the user has clicked either the 'upload' or the 'delete' button --->
	...
	<!--- [8] The user has clicked 'UploadFileButton', process the new file. --->
		...
		<!--- [9] End if --->
			...
			<cfif ListFindNoCase("gif,jpg,jpeg,png,bmp",File.ServerFileExt) AND NOT Find(File.ServerFile, ";")>
				...
				<!--- [11] Update the file column for the record with the just-constructed image file name. --->
					<cfif TheTableType EQ 'Local'>
						...
						<cfquery name="AddImageToRecord" datasource="#Request.DS#" username="#Request.DSuser#" password="#Request.DSpass#">
							UPDATE #TheTableName# SET
				...
				<!--- [11] Else if --->
					<cfelseif TheTableType EQ 'Foreign'>
						...
						<cftransaction isolation="SERIALIZABLE">
							...
							<!--- get record id --->
						    <cfquery name="New_ID" datasource="#Request.DS#" username="#Request.DSuser#" password="#Request.DSpass#">
			...
			<cfif len(form['Photo_SM_' & TheRecordID])>
