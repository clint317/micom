...
<cfoutput>		
	...
	<tr>
		...
		<select name="editorial" size="1" class="formfield">
			...
			<option value="NA">customers</option>
...
<cfif qry_get_reviews.recordcount gt 0>					
	...
	<cfset reviewList = ListAppend(reviewList, Review_ID)>
		...
		<tr>
			...
			<a href="#Request.StoreSelf#?fuseaction=product.reviews&do=display&Review_ID=#Review_ID##request.token2#" target="store"><strong>#LEFT(title, 50)#</strong></a><br/>
				
				<strong>re:</strong> <a href="#Request.StoreSelf#?fuseaction=product.display&product_ID=#product_ID##request.token2#" target="store">#LEFT(product_Name, 50)#</a><br/>

			<em>by <cfif Anonymous is 1>a member<cfelseif user_ID><a href="#Request.StoreSelf#?fuseaction=product.reviews&do=list&uid=#user_ID##request.token2#" target="store"><cfif len(anon_name)>#anon_name#<cfelse>#username#</cfif></a><cfelse>#anon_name#</cfif>
