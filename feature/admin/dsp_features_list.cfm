...
<cfset fieldlist="feature,uid,username,search_string,feature_type,accesskey,display_status,highlight,order,cid">
...
<cfset session.formKeys.featureList = formkey>		
...
<!---- Category Browse --------->		
	<tr>
		...
		<tr>
			<td colspan="8" style="BACKGROUND-COLOR: ###Request.GetColors.outputHBgcolor#;"><img src="#Request.ImagePath#spacer.gif" alt="" height="1" width="1" /></td>
		</tr>
...
<cfif qry_get_Features.recordcount gt 0>					
	<cfloop query="qry_get_Features" startrow="#PT_StartRow#" endrow="#PT_EndRow#">
		<tr>
			...
			<td colspan="2"><a href="#Request.StoreSelf#?fuseaction=feature.display&feature_ID=#feature_ID##Request.Token2#"  target="store">#left(name,41)#<cfif len(name) gt 41>...</cfif></a><br/><span class="formtextsmall"><cfif len(author)>#left(author,15)#<cfif len(author) gt 15>...</cfif></cfif><cfif len(author) and len(created)>, </cfif>#DateFormat(created,'mm/dd/yy')#  <cfif len(copyright)>&copy;#left(copyright,12)#<cfif len(copyright) gt 12>...</cfif></cfif></span>
		...
		<cfelse><font color="green">current</font>
			...
			<td><a href="#self#?fuseaction=Feature.admin&Feature=copy&dup=#Feature_id#&formkey=#Hash(formkey,"SHA-256")##Request.Token2#">
			copy</a></td>	
