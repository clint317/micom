...
<cfif qry_get_reviews.recordcount gt 0>					
	<cfloop query="qry_get_reviews" startrow="#PT_StartRow#" endrow="#PT_EndRow#">
		<tr>
			...
			<td colspan="2">
				<a href="#Request.StoreSelf#?fuseaction=feature.reviews&do=display&Review_ID=#Review_ID##request.token2#" target="store"><strong>#HTMLEditFormat(LEFT(title, 27))#</strong></a>
			<br/><a href="#Request.StoreSelf#?fuseaction=feature.display&Feature_ID=#Feature_ID##request.token2#" target="store">#HTMLEditFormat(LEFT(Feature_Name, 30))#</a>	
			...
			<td>#Editorial#<br/>
			<a href="#self#?fuseaction=feature.admin&review=list&uid=#user_ID##request.token2#">#HTMLEditFormat(username)#</a></td>
