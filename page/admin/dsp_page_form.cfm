...
<cfset fieldlist="page_url,Display,Page_title,PageAction,Page_name,sm_title,Lg_image,Lg_title,catcore_ID,Passparam,PageText,Priority,system,href_attributes,accesskey,parent_id,title_priority,metadescription,keywords,titletag">	
...
<cfswitch expression="#do#">
	...
	<cfcase value="add">
		...
		<cfset attributes.page_color_id = ''>
	...
	<cfcase value="edit">
		...
		<cfloop list="#fieldlist#" index="counter">
			<cfset attributes[counter] = qry_get_page[counter][1]>
		</cfloop>
		<cfset attributes.page_color_id = qry_get_page.color_id>			
...
<a href="JavaScript: newWindow = openWin('#self#?fuseaction=home.admin&select=image&fieldname=Sm_Title&fieldvalue=#attributes.Sm_Title#', 'images', 'width=850,height=700,toolbar=0,location=0,directories=0,status=1,menuBar=0,scrollBars=0,resizable=1'); newWindow.focus()" class="formtextsmall">Image Manager</a>
			</td></tr>
	...
	<!--- Display --->
		<tr>
			...
			<td></td>
			<td><input type="radio" name="Page_Display" value="1" #doChecked(attributes.Display)# /> Yes  
			&nbsp;&nbsp;<input type="radio" name="Page_Display" value="0" #doChecked(attributes.Display,0)# /> No
			</td></tr>
 ...
 <!--- Access Key --->
	...
	<!--- Large Image --->
		<tr>
			...
			<td></td>
			<td><input type="text" name="Lg_Image" size="33" maxlength="100" class="formfield" value="#attributes.Lg_Image#"/> <a href="JavaScript: newWindow = openWin('#self#?fuseaction=home.admin&select=image&fieldname=Lg_Image&fieldvalue=#attributes.Lg_Image#', 'images', 'width=850,height=700,toolbar=0,location=0,directories=0,status=1,menuBar=0,scrollBars=0,resizable=1'); newWindow.focus()" class="formtextsmall">Image Manager</a>
			</td></tr>			
	...
	<!--- Long Description --->
		<tr>
			<td colspan="3" align="center">							
				...
				<cfmodule 
					template="../../customtags/wysiwyg/fckeditor.cfm" 
					instanceName="PageText"
					height="500" 
					config="#config#"
					Value="#Application.objGlobal.doImgPaths(attributes.PageText)#"
					/>
			</td>
			</tr>
	...
	<!--- Parameters --->
		<tr>
			...
			<cfif isdefined("qry_get_page.passparams") and len(qry_get_page.passparams)>
				<span class="formtextsmall">
					<br/><strong>#qry_get_page.PassParams#</strong>
					<br/>These params, which should be self describing, are "option" switches for this page.
					<br/>(tech: they are listed in the <a href="#self#?fuseaction=home.admin&CatCore=edit&CatCore_ID=#attributes.Catcore_ID#">template</a> which also points to the template file.)
				</span>
			</cfif>
 ...
 <!--- Keywords --->
	...
	<!--- Custom palette --->
		<tr>
			...
			<td></td>
	 		<td>
				<select name="page_color_id" size="1" class="formfield">
					<option value="" #doSelected(attributes.page_color_id,'')#>Default Palette</option>
					<cfloop query="qry_Get_Colors">
   						<cfif qry_Get_Colors.color_id IS NOT request.appsettings.color_id>
						<option value="#color_id#" #doSelected(attributes.page_color_id,qry_Get_Colors.color_id)#>#palette_name#</option>
						</cfif>
					</cfloop>
				</select>
			</td></tr>
