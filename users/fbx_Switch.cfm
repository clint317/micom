...
Users.unlock - a form called by secure.cfm to validate a user email address when email 
	...
	user can click or enter into a web browser: URL or href= "#request.appsettings.secureURL##self#?fuseaction=users.unlock&email=#users.email#&emaillock=#users.emaillock#"
...
<cfelseif CompareNoCase(fusebox.fuseaction, "loginbox") IS 0>
	...
	<cfif Session.User_ID or isdefined("attributes.submit_logout")>
		...
		<cfif isdefined("attributes.submit_logout") AND get_User_Settings.UseRememberMe 
			...
			<cflocation url="#Request.SecureSelf#?fuseaction=users.purgecookie&amp;xfa_logout=#attributes.xfa_logout_successful##Request.AddToken#" addtoken="No">
		</cfif>				
		...
		<cfif fuseaction is "users.purgecookie" AND isDefined("cookie.#request.ds#_Username")>
			<cfset attributes.Username = cookie[request.ds & '_Username']>
			<cfset attributes.password = cookie[request.ds & '_password']>
			<cfparam name="attributes.xfa_LoginAction" default="fuseaction=users.manager">
...
<cfelseif CompareNoCase(fusebox.fuseaction, "purgecookie") IS 0>
	...
	<cfparam name="attributes.xfa_logout" default="">
...
<cfelseif CompareNoCase(fusebox.fuseaction, "register") IS 0>
	...
	<cftry><cfinclude template="do_register.cfm"><cfcatch type="any"><cfdump var='#cfcatch#' label='cfcatch' abort=true /></cfcatch></cftry>
...
<cfelseif CompareNoCase(fusebox.fuseaction, "account") IS 0>
	<cfinclude template="qry_get_user.cfm">
	<cfset attributes.xfa_success = "fuseaction=users.manager">
	...
	<cfset Webpage_title = "Address Book">
  	...
  	<cfif not isdefined("attributes.xfa_success")>
    	<cfif attributes.show is "billto" or attributes.show is "shipto">
   			<cfset attributes.xfa_success = "fuseaction=shopping.checkout">
  		<cfelse>
     		<cfset attributes.xfa_success = "fuseaction=users.manager">
  		</cfif>
  	</cfif>
...
<cfelseif CompareNoCase(fusebox.fuseaction, "ccard") IS 0>
	...
	<cfif IsDefined("attributes.step") and attributes.step IS "shift4_i4go">
		<cfinclude template="../shopping/checkout/creditcards/shift4/act_shift4i4go.cfm">
	</cfif>
...
<!--- Admin function for logging in to add orders as another user --->
<cfelseif CompareNoCase(fusebox.fuseaction, "loginAsAnother") IS 0>
	<!--- Check that the user has proper permission to access this page --->
	<cfmodule template="../access/secure.cfm"
		keyname="shopping" requiredPermission="1024" />	
	<cfif ispermitted OR isDefined("Session.AnotherUserLogin")>
		<cfinclude template="login/dsp_loginAsAnother.cfm">	
	<cfelse>
		<cfinclude template="../access/dsp_no_permission.cfm">
	</cfif>
	
<!--- This function is used to unsubscribe a user, using the unsub code passed on the URL --->
<cfelseif CompareNoCase(fusebox.fuseaction, "autounsub") IS 0>
	<cfinclude template="act_run_autounsub.cfm">	
...
<cfelse>
	<cfmodule template="../#thisSelf#" fuseaction="page.pageNotFound">
