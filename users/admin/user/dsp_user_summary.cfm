...
<strong>User:</strong> <cfif NOT get_User_Settings.EmailAsName><a href="#self#?fuseaction=users.admin&user=edit&UID=#attributes.uid#&XFA_success=#URLEncodedFormat(cgi.query_string)##Request.Token2#">#qry_get_user.Username#</a><br/>
...
<cfif len(qry_get_user.emailLock) AND qry_get_user.emailLock is NOT "verified">
					...
					[ <a href="#self#?fuseaction=users.admin&email=write&uid=#attributes.uid#&MailText_ID=2&XFA_success=#URLEncodedFormat(cgi.query_string)#">send code</a> ]<br/>
</cfif>		
[ <a href="#self#?fuseaction=users.admin&customer=list&uid=#attributes.uid#&show=&XFA_success=#URLEncodedFormat(cgi.query_string)##Request.Token2#">Address Book</a> ]
		</td>
	</tr>
...
<!--- 	<cfif request.appSettings.AllowAffs> --->
	<tr><td><strong>Affiliate:</strong>
		...
		</td><td>[ <a href="#self#?fuseaction=users.admin&user=affiliate&uid=#qry_get_user.user_ID#&XFA_success=#URLEncodedFormat(cgi.query_string)##Request.Token2#">Edit</a> ] 
		[ <a href="#self#?fuseaction=shopping.affiliate&do=report&UID=#qry_get_user.user_ID##Request.Token2#" target="_blank">Report</a> ]
		<cfelse>Not an Affiliate</td>
		<td>[ <a href="#self#?fuseaction=users.admin&user=affiliate&uid=#qry_get_user.user_ID#&XFA_success=#URLEncodedFormat(cgi.query_string)##Request.Token2#">Create</a> ]</cfif>
	</td></tr>
<!--- 	</cfif>		 --->	
	...
	<tr>
		<td valign="top"><strong>Billing Address:</strong>
			...
			<cfif qry_get_User.customer_id gt 0>
				...
				[ <a href="#self#?fuseaction=users.admin&customer=edit&Customer_ID=#qry_get_User.customer_id#&XFA_success=#URLEncodedFormat(cgi.query_string)##Request.Token2#">Edit</a> ]
			<cfelse>
				No billing address.
			</cfif>
			</blockquote>
		...
		<cfif  get_User_Settings.UseShipTo>
			...
			<cfif qry_get_user.shipto>
				<cfif qry_get_user.shipto and qry_get_user.shipto is not qry_get_user.customer_id>
					...
					<cfinclude template="../../manager/put_customer.cfm">
					[ <a href="#self#?fuseaction=users.admin&customer=edit&Customer_ID=#qry_get_User.shipto#&XFA_success=#URLEncodedFormat(cgi.query_string)##Request.Token2#">Edit</a> ]
				<cfelse>
...
<cfquery name="qry_get_Memberships"  datasource="#Request.DS#" username="#Request.DSuser#" password="#Request.DSpass#">
	...
	<tr>
		<td valign="top"><strong>Credit Card on file:</strong>
			...
			<span class="formerror"><strong>NOT ACTIVE: </strong><a href="#self#?fuseaction=users.admin&user=authorize&UID=#attributes.UID#&XFA_success=#URLEncodedFormat(cgi.query_string)##request.token2#" class="formerror">Authorize Now</a></span>					
			</cfif>
		...
		<td valign="top">
			...
			<cfloop query="qry_get_Memberships">
				<a href="#self#?fuseaction=access.admin&Membership=edit&Membership_ID=#membership_ID#&XFA_success=#URLEncodedFormat(cgi.query_string)##request.token2#">#product#</a><br/>
				...
				<cfif NOT Valid><br/>
				<a href="#self#?fuseaction=access.admin&membership=list&show=all&valid=0&XFA_success=#URLEncodedFormat(cgi.query_string)##request.token2#" class="formerror"><strong>VALIDATE</strong></a>
				</cfif>
				<br/><br/>
			</cfloop>	
			</blockquote>
	...
	<tr><td colspan="2"><strong>Other:</strong> <a href="#self#?fuseaction=product.reviews&do=list&uid=#qry_get_user.user_ID#" target="_blank">Product Reviews</a> | 

<a href="#self#?fuseaction=feature.reviews&do=list&uid=#qry_get_user.user_ID#" target="_blank">Feature Article Comments</a> </td></tr>
...
<strong>Rep:</strong> #qry_get_account.Rep#<br/></cfif>
		[ <a href="#self#?fuseaction=users.admin&account=edit&account_ID=#account_ID#&XFA_success=#URLEncodedFormat(cgi.query_string)##Request.Token2#">Edit Account</a> ]</td>
...
style="BACKGROUND-COLOR: ##F4F4F6; COLOR: ###Request.GetColors.OutputtTEXT#;"></cfoutput>
	...
	<cfelse>
		...
		<tr>
			<td valign="top"><a href="#self#?fuseaction=access.admin&Membership=edit&Membership_ID=#membership_ID#&XFA_success=#URLEncodedFormat(cgi.query_string)##Request.Token2#">Edit #membership_ID#</a></td>
	
			<td valign="top">#Prod_Type#: <cfif prod_type is "download" and access_count gt access_used AND Valid AND (NOT isDate(Expire) OR DateCompare(Expire, Now()) GT 0)><a href="#self#?fuseaction=access.download&ID=#membership_id#&XFA_success=#URLEncodedFormat(cgi.query_string)##Request.Token2#">#product#</a>
...
<cfelseif valid is 0><a href="#self#?fuseaction=access.admin&membership=approve&membership_id=#membership_id#&XFA_success=#URLEncodedFormat(cgi.query_string)##Request.Token2#">Validate</a><cfelseif isDate(start) AND DateCompare(start,now()) lt 1><span style="color:green; font-weight:bold;">Current</span><cfelse><span style="color:green;">Future</span></cfif></td>
		</tr>
		...
		<tr>
			...
			&nbsp;&nbsp; Process now: <a href="#self#?fuseaction=access.admin&membership=bill_recurring&membership_ID=#membership_ID#&offline=1&XFA_success=#URLEncodedFormat(cgi.query_string)##request.token2#">OFFLINE</a> | <a href="#self#?fuseaction=access.admin&membership=bill_recurring&membership_ID=#membership_ID#&XFA_success=#URLEncodedFormat(cgi.query_string)##request.token2#">ONLINE</a>
			<br/><br/></td>
		</tr>
		</cfif>
...
<table width="100%" cellspacing="5" cellpadding="0" class="formtext" border="0" style="BACKGROUND-COLOR: ##F4F4F6; COLOR: ###Request.GetColors.OutputtTEXT#;">
	...
	<cfelse>
		...
		<tr>
			<td valign="top"><a href="#self#?fuseaction=shopping.admin&order=display&order_no=#GetOrders.Order_No#&XFA_success=#URLEncodedFormat(cgi.query_string)##Request.Token2#">Edit #(GetOrders.Order_No + Get_Order_Settings.BaseOrderNum)#</a></td>
