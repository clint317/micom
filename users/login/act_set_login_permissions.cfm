...
<cfif len(qry_get_user.permissions)>
	<cfloop index="thiskey" list="#qry_get_user.permissions#" delimiters=";">
		...
		<cfelse>
			<cfset Keys[listGetAt(thiskey, 1, '^')] = ''>
		</cfif>		
	</cfloop>
</cfif>
...
<cfquery name="GetMemberships" datasource="#Request.DS#" username="#Request.DSuser#" password="#Request.DSpass#">
...
<cfif GetMemberships.recordcount>
	...
	<cfelse>
		<cfset Keys.contentkey_list = valuelist(GetMemberships.Accesskey_ID)>
	</cfif>
...
<cfloop index="thiskey" list="#StructKeyList(Keys)#">
	<cfset sessionkeys = listappend(sessionkeys,'#thiskey#^#Keys[thiskey]#',';')>
</cfloop>		
