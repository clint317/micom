...
<!--- These generally do not need to be changed but may be adjusted to run CFWebstore inside other applications --->
<cfparam name="self" default="index.cfm">
<cfparam name="request.self" default="#self#">
...
<cfset Request.StoreSelf = Request.StoreURL & thisSelf>
		
<cfif LEFT(self, 5) IS NOT "https" AND (CGI.HTTPS EQ "ON" OR CGI.server_port_secure IS 1)>
	<cfset self = Request.SecureURL & self>
	<cfset request.self = self>
<cfelseif LEFT(self, 4) IS NOT "http">
	<cfset self = Request.StoreURL & self>			
	<cfset request.self = self>
</cfif>
...
<cfif fusebox.IsHomeCircuit>
	...
	<cfif not IsDefined("ThisTag.ExecutionMode") AND NOT StructKeyExists(fusebox,'customtagcall')>
		...
		<cfscript>
			...
			//Set imagepath
			Request.ImagePath = Request.StorePath & Request.AppSettings.defaultimages & "/";
				
		</cfscript>
		
		<!--- Set location of error handler pages 	 ---> 	
		<cfif NOT Request.DevelopmentMode>
		...
		// Check for invalid fuseactions 
			if (isDefined('invalidFA') OR ListLen(attributes.fuseaction, ".") LT 2) {
		...
		<cfif NOT structKeyExists(Application,"reload_CFCs") OR Request.DevelopmentMode>
	...
	<!--- Set cookies for session to be cleared when browser closed --->	
		<cfif not IsDefined("Cookie.CFID") AND IsDefined("Session.CFID")>
			<!--- If store path and secure path not the same, use traditional cookies --->
			<cfif Request.SharedSSL>
         		<cfcookie name="CFID" value="#Session.CFID#">
          		<cfcookie name="CFTOKEN" value="#Session.CFTOKEN#">
			<!--- Otherwise use httponly cookies for additional protection against spoofing --->
			<cfelse>
		...
		<!--- Initialize Form keys structure, for CSRF attack checks --->
        <cfif NOT StructKeyExists(Session,"formKeys")>
             <cfset Session.formKeys = StructNew()>
        </cfif>
		...
		<cfif Session.BasketNum is 0>
			<cfinvoke component="#Request.CFCMapping#.shopping.cart" method="doNewBasket" returnvariable="Session.BasketNum">
		...
		<cfif translatepath>
			...
			<cfif len(Request.Query_String)>
				...
				<cfset Request.LoginURL = "#path_info#?#Request.Query_String#">
			<cfelse>
				<cfset Request.currentURL = "#self##path_info##Request.Token1#">
				<cfset Request.LoginURL = "#path_info#?dologin=yes">
			</cfif>
		...
		<cfelse>
			...
			<cfset Request.LoginURL = Request.Query_String>
		...
		<!--- Clear cookies/session for users coming from external links or spoofing session. Setting when not using shared SSL --->
 		<cfif NOT Request.SharedSSL AND isDefined("URL.CFID") OR isDefined("Form.CFID")>
			<!--- Clear only if not in the checkout process --->
		...
		<cfif clearsession>			
		    ...
		    <cfset attributes.xfa_logout_successful = Request.Query_String>
		...
		<cfif Request.secure_cookie AND ListGetAt(attributes.fuseaction,2,".") IS "admin" AND CGI.SERVER_PORT NEQ 443 >
			...
			<cfheader name="Location" value="#doReturn(Request.LoginURL)##Request.AddToken#">
		</cfif> 
