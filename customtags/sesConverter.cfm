...
This CF script can be CFINCLUDED or called as a custom tag.  
Here's some code to use for CFINCLUDING:
...
Fusebox v3 - index.cfm
Fusebox v1 or 2 - app_globals.cfm

If you are not using Fusebox, application.cfm will work great.


2) BASEHREF TAG:

SES URLs will not work unless you use the basehref tag.  This is
not a CF tag, it's standard HTML.  SES URLs mess up your relative
links when this tag is not used.  

In the <HEAD> section of your templates (layout files for Fusebox),
include this code:
...
You can change the name of the variable used by modifying SESrBaseName
(see above in the code used to cfinclude sesConverter).  By default,
this variable is set to "baseHREF."


	</responsibilities>
	<properties>
		<property name="version" value="1.6" />
		<property name="lastUpdated" value="22-October-2002" />
...
if ((Len(currentPath)) AND (Len(cgi.script_name) GT Len(currentPath))) {
...
s_Prefix = "http";
if (CGI.HTTPS EQ "ON" OR CGI.server_port_secure IS 1 OR CGI.SERVER_PORT IS 443) {
	s_Prefix = "https";
}
s_Port = "";
