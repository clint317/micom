...
<cfquery name="getUser" datasource="#Request.DS#" username="#Request.DSuser#" password="#Request.DSpass#" maxrows="1">
...
<cfquery name="getEmailText"  datasource="#Request.DS#" username="#Request.DSuser#" password="#Request.DSpass#" maxrows="1">
...
<cfargument name="UserRow" type="numeric" required="No" default="0" hint="Current row in the user query being processed.">
...
<cfscript>
	...
	if (isDefined("arguments.UserQuery") AND arguments.UserRow GT 0) {
		str = replaceNoCase(str,'%Email%',arguments.UserQuery['Email'][arguments.UserRow],'All');
		str = replaceNoCase(str,'%Username%',arguments.UserQuery['Username'][arguments.UserRow],'All');
		str = replaceNoCase(str,'%Password%',arguments.UserQuery['Password'][arguments.UserRow],'All');
		str = replaceNoCase(str,'%EmailLock%',arguments.UserQuery['EmailLock'][arguments.UserRow],'All');
		str = replaceNoCase(str,'%LastLogin%',dateformat(arguments.UserQuery['LastLogin'][arguments.UserRow],'mm/dd/yyyy'),'All');
		str = replaceNoCase(str,'%Created%',dateformat(arguments.UserQuery['Created'][arguments.UserRow],'mm/dd/yyyy'),'All');	
	}
...
<cfreturn str>
 ...
 <!---------------------- BEGIN IMAGE PATHS FUNCTION ---------------------->
 <cffunction name="doImgPaths" access="public" output="no" displayname="Used to return editor content with the image paths replaced.">
 	
	<cfargument name="Content" required="true" type="string">
	
	<cfscript>
		var txtReturn = '';
		var paramMatch = '(<img[[:space:]]+.*?src=")(#Request.AppSettings.DefaultImages#.*?".*?>)';
		
		txtReturn = ReReplaceNoCase(arguments.Content, paramMatch, '\1#Request.StorePath#\2', 'ALL');
...
<cffunction name="fieldName" returntype="string" displayname="I check for a field name remap" hint="If the structure request.fieldMapping exists and a matching key name, I return the new value." output="No">
	<cfargument name="value" type="string" required="Yes" />
	
	<cfif IsDefined("request.fieldMap") and StructKeyExists(request.fieldMap,value)>
		<cfreturn request.fieldMap[value]>
	<cfelse>
		<cfreturn value>
	</cfif>
</cffunction>
