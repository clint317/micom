...
<cfargument name="TotalValue" type="numeric" required="Yes" hint="Total cost/value of the order">
...
v.FinalValue = arguments.TotalValue;

//Check if TempArray has items (oversized packages). If so, subtract weight/value from the total weight/value
	if (ArrayLen(PackageArray) IS NOT 0) {
		...
		for (i=1;i LTE v.numitems; i=i+1) {
			...
			v.FinalValue = v.FinalValue - v.ItemStruct.Value;
		}
	}
	...
	if (v.FinalShip GT 0 OR ArrayLen(PackageArray) IS 0) {
		...
		if (arguments.MaxWeight LT v.FinalShip) {
			...
			v.valueperpack = Round(v.FinalValue/v.numPacks*100)/100;
		}
		else {
			v.numPacks = 1;
			v.weightperpack = v.FinalShip;
			v.valueperpack = v.FinalValue;
			}	
		...
		v.ItemShip.Value = v.valueperpack;
...
<cffunction name="doShippingData" description="Do Shipping Data" hint="Takes the customer address and merchant information and shipping data and creates a final array of shipping information" access="public" output="No" returntype="array">
	...
	BaseShipping.TotalValue = Session.CheckoutVars.TotalCost - Session.CheckoutVars.DropShipValue;
	...
	if (BaseShipping.TotalWeight IS NOT 0) {
		...
		ShipInfo.TotalValue = BaseShipping.TotalValue;
	...
	<cfif NOT StructIsEmpty(Session.CheckoutVars.Dropshippers)>
		...
		<cfloop collection="#Session.CheckoutVars.Dropshippers#" item="vendor">
			...
			<cfscript>
				...
				DropShipper.TotalValue = Session.CheckoutVars.Dropshippers[vendor].TotalValue;
				...
				ShipInfo.TotalValue = DropShipper.TotalValue;
...
<cffunction name="getFedExMethods" output="No" hint="Returns a query with the full list of FedEx Methods." access="public">
	...
	<cfquery name="qryFedExMethods" datasource="#Request.DS#" username="#Request.DSuser#" 
	password="#Request.DSpass#" cachedwithin="#Request.Cache#">
...
<cffunction name="getUPSMethods" output="No" hint="Returns a query with the list of active UPS Methods." access="public">
	...
	<cfquery name="GetOrigin" datasource="#Request.DS#" username="#Request.DSuser#" password="#Request.DSpass#" cachedwithin="#Request.Cache#">
		SELECT Origin FROM #Request.DB_Prefix#UPS_Settings
	</cfquery>
	
	<cfquery name="qryUPSMethods" datasource="#Request.DS#" username="#Request.DSuser#" password="#Request.DSpass#" cachedwithin="#Request.Cache#">
		SELECT ID, Name, Used, Priority, #GetOrigin.Origin#Code AS Code, 'UPS' AS Shipper 
...
<cffunction name="getUSPSMethods" output="No" hint="Returns a query with the list of active US Postal Methods." access="public">
	...
	<cfquery name="qryUSPSMethods" datasource="#Request.DS#" username="#Request.DSuser#" password="#Request.DSpass#" cachedwithin="#Request.Cache#">
		SELECT *, 'USPS' AS Shipper 
		FROM #Request.DB_Prefix#USPSMethods
		WHERE Used = 1
		ORDER BY Priority
	</cfquery>
...
<cffunction name="getIntShipMethods" output="No" hint="Returns a query with the list of Intershipper Methods." access="public">
	...
	<cfquery name="qryIntShipTypes" datasource="#Request.DS#" username="#Request.DSuser#" 
	password="#Request.DSpass#" cachedwithin="#Request.Cache#">
...
<cffunction name="getCustomMethods" output="No" hint="Returns a query with the list of Custom Shipping Methods." access="public">
	...
	<cfquery name="qryShipMethods" datasource="#Request.DS#" username="#Request.DSuser#" password="#Request.DSpass#" cachedwithin="#Request.Cache#">
		SELECT CM.*, CM.Name AS Code, '' AS Shipper 
...
<cffunction name="getFedExSettings" output="No" hint="Returns a query with the list of FedEx Settings." access="public">
	...
	<cfquery name="qryFedExSettings" datasource="#Request.DS#" username="#Request.DSuser#" password="#Request.DSpass#" cachedwithin="#Request.Cache#">
...
<cffunction name="getUPSSettings" output="No" hint="Returns a query with the list of UPS Settings." access="public">
	...
	<cfquery name="qryUPSSettings" datasource="#Request.DS#" username="#Request.DSuser#" password="#Request.DSpass#" cachedwithin="#Request.Cache#">
...
<cffunction name="getUSPSSettings" output="No" hint="Returns a query with the list of USPS Settings." access="public">
	...
	<cfquery name="qryUSPSSettings" datasource="#Request.DS#" username="#Request.DSuser#" password="#Request.DSpass#" cachedwithin="#Request.Cache#">
...
<cffunction name="getIntShipSettings" output="No" hint="Returns a query with the list of USPS Settings." access="public">
	...
	<cfquery name="qryIntShipSettings" datasource="#Request.DS#" username="#Request.DSuser#" 
	password="#Request.DSpass#" cachedwithin="#Request.Cache#">
...
<cffunction name="getCustomSettings" output="No" hint="Returns a query with the list of custom shipping Settings." access="public">
	...
	<cfquery name="qryCustomSettings" datasource="#Request.DS#" username="#Request.DSuser#" password="#Request.DSpass#" cachedwithin="#Request.Cache#">
...
<cffunction name="getDropShippers" output="No" hint="Returns a query with the list of drop shipper addresses." access="public">
	...
	<cfquery name="qryDropShippers" datasource="#Request.DS#" username="#Request.DSuser#" password="#Request.DSpass#">
		SELECT A.Account_ID, C.Company, C.Address1, C.City, C.State, C.Zip, C.Country 
