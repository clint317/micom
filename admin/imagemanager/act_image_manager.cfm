...
<cfif len(attributes.user) AND attributes.user is NOT 0>
	<cfparam name="attributes.topdir" default="#Request.ImagePath#User#attributes.User#">
<cfelse>
	<cfparam name="attributes.topdir" default="#Request.ImagePath#">
</cfif>
...
<cfscript>
	...
	theDirectory = TopDirectory & Request.AppSettings.defaultimages & attributes.dirname;
...
<cfif attributes.dirlist neq 1>
	...
	<cfif isDefined("attributes.newfolder")>
		<!--- CSRF Check --->
		<cfset keyname = "imageManager">
	...
	<!--- Upload the file --->
	<cfif isDefined("attributes.newimage")>
		<!--- CSRF Check --->
		<cfset keyname = "imageManager">
		<cfinclude template="../../includes/act_check_csrf_key.cfm">
		...
		<cftry>
			...
			<!--- Check that the file extension is in the allowed list before continuing, and no semicolons (hack attempts) --->
			<cfif ListFindNoCase(showfiletypes,File.ServerFileExt) AND NOT Find(File.ServerFile, ";")>
			...
			<cfelse>
				<cfset message = "There was a problem with your upload. Make sure that you are uploading an appropriate file type and do not have any invalid characters in the filename.">
	...
	<cfset session.formKeys.imageManager = formkey>
	...
	<cfif attributes.fieldname is "fckeditor">
		var fullimage = '#attributes.topdir#' + image ;
	...
	<!--- This will get appended to the body tag --->
...
<table width="100%" border="0" cellspacing="0" cellpadding="2" class="formtext" style="border: 1px solid ###Request.GetColors.InputHBgcolor#;">
	<tr bgcolor="###Request.GetColors.InputHBgcolor#">
		<th align="left" class="boxtitle" colspan="2">
			<font color="###Request.GetColors.InputHText#">#attributes.type# Manager &nbsp;&nbsp;&nbsp;&nbsp;<img src="#Request.ImagePath#icons/openfolder.gif" alt="Open Folder" border="0" align="bottom" /><span style="font-size:12px">#attributes.dirname#<cfif isdefined("attributes.filename")>/#attributes.filename#</cfif></span></font>
		</th>
	</tr>
	<tr >
		<td width="200" rowspan="3" valign="top">
		<iframe src="#actionURL#&dirlist=1&dirname=#URLEncodedFormat(attributes.dirname)##Request.Token2#" frameborder="0" width="200" height="550"></iframe>
		</td>
		<td align="right" height="20"><a href="#actionURL#&do=newfolder&dirname=#attributes.dirname#" #doMouseover('Create New Folder')#><img src="#Request.ImagePath#icons/trimfolder.gif" alt="" border="0" align="bottom" /> <strong>New Folder</strong></a> &nbsp; <a href="#actionURL#&do=newimage&dirname=#attributes.dirname#" #doMouseover('Upload New #attributes.type#')#><img src="#Request.ImagePath#icons/trimimage.gif" alt="" border="0" /> <strong>New #attributes.type#</strong></a> &nbsp;</td>
	</tr>

	<tr>
    	<td width="400" align="center" height="475">
...
<!--- New Image ---->		
	...
	<b>#buttonText# to Upload:</b><br/> <input type="file" name="FileName" size="35" class="formfield"/>
	</td></tr>
...
<!--- View Image ---->	
	<cfelseif attributes.do is "viewimage">
		...
		<cfif attributes.type is "image">
			<iframe src="#imageurl#/#attributes.filename#" width="550" height="450" frameborder="0"></iframe>
	 ...
	 <table width="98%" border="0"  cellspacing="0" cellpadding="2" align="center" class="formtext">
        <tr> 
          <td><b>#attributes.type#:</b> #attributes.filename# &nbsp;&nbsp; 
		  	<a href="#actionURL#&do=delimage&dotheDelete=yes&dirname=#attributes.dirname#&fn=#attributes.filename#&formkey=#Hash(formkey,"SHA-256")##Request.Token2#" #doMouseover('Delete This #attributes.type#')# onclick="return confirm('Are you sure you want to delete this file?');">
		  	<img src="#Request.ImagePath#icons/delete.gif" alt="" border="0" align="bottom" /><strong>Delete</strong></a>
		  </td> 
          <td align="right" width="100"><button onclick="javascript:updatefieldname('<cfif len(attributes.dirname)>#Replace(attributes.dirname, "/", "", "ONE")#/</cfif>#attributes.filename#');" class="formbutton"> #buttonText#</button></td>
        </tr>
	</cfif>
...
<!---
	...
	<cfif Len(attributes.dirname)>
		...
		<a href="#ActionURL#&dirname=#dirup#" target="_parent" style="font-size:11px; font-family: Arial, Helvetica, sans-serif;" #doMouseover('Change Directory')#><img src="#Request.ImagePath#icons/upfolder.gif" border="0" alt="" align="middle" width="18" height="15" /> [ Up One Level ]</a><br/>
	</cfif>
	...
	<cfloop index="i" from="1" to="#numdirs#">
		...
		<a href="#ActionURL#&dirname=#attributes.dirname#%2F#URLEncodedFormat(name)#" target="_parent" style="font-size:11px; font-family: Arial, Helvetica, sans-serif;" #doMouseover('Click to Open')#><img src="#Request.ImagePath#icons/folder.gif" alt="Open Folder" border="0" align="middle" width="18" height="15" />#name#</a><br/>
	</cfloop> 
