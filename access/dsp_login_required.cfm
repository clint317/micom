...
<div id="dsploginrequired" class="access">
...
<cfif FindNoCase(".admin", attributes.xfa_success)>
	...
	if (top !=self)
		<cfif isDefined("attributes.cache")>
			top.location=<cfoutput>'#Request.secureSelf#?fuseaction=home.admin#Request.AddToken#&redirect=yes';</cfoutput>
		<cfelse>
...
<table width="100%" cellpadding="0" cellspacing="3" class="mainpage">
	<tr>
		...
		<cfoutput>
		<form action="#XHTMLFormat('#Request.secureSelf#?fuseaction=users.#request.reg_form#&xfa_success=#urlEncodedFormat(attributes.xfa_success)##request.token2#')#" method="post"></cfoutput>
