...
<cfset fieldlist = "membership,isexpired,isused,valid,user,membership_type,product,accesskey,show">
...
<table width="100%" cellspacing="4" border="0" cellpadding="0" class="formtext" style="color: ###Request.GetColors.OutputTText#;">	
	...
	<tr>
		<td colspan="9" style="BACKGROUND-COLOR: ###Request.GetColors.outputHBgcolor#;"><img src="#Request.ImagePath#spacer.gif" alt="" height="1" width="1" /></td>
		</tr>	
...
<cfif qry_get_memberships.recordcount gt 0>
	...
	<cfset session.formKeys.billRecurring = formkey>
	<cfset session.formKeys.approveMember = formkey>
	...
	<cfloop query="qry_get_Memberships" startrow="#PT_StartRow#" endrow="#PT_EndRow#">
		<tr>
			...
			<cfelseif valid is 0><a href="#self#?fuseaction=access.admin&membership=approve&membership_id=#membership_id#&formkey=#Hash(formkey,"SHA-256")##Request.Token2#">Validate</a><cfelseif len(start) AND DateCompare(start,now()) lt 1><span style="color:green; font-weight:bold;">Current</span><cfelse><span style="color:green;">Future</span></cfif></td>
			</tr>
