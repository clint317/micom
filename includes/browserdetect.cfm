...
function testQueryString(browserType,query_string){ 
	...
	if (StructKeyExists(fusebox, 'fuseaction') AND fusebox.fuseaction IS 'admin') {
		BrowserInfo.hackattempt = checkforAttack('admin');
	}
	else {
		BrowserInfo.hackattempt = checkforAttack('public');
	}
	...
	if (FindNoCase("http", query_string) AND NOT FindNoCase("home.email", query_string) ) {
		BrowserInfo.hackattempt="yes";
	}
...
<cfif BrowserInfo.hackattempt>
	<cfoutput>
	<!-- start includes/browserdetect.cfm -->
	<br/><br/>Sorry, your page request appears to be a hack attempt and processing has been halted.<br/><br/> If you arrived at this site via another location, it may have had some code intended to try and find sensitive information. <br/><br/>If you believe this message was received in error, return to the previous page and change the text you entered. <br/><br/>If you continue to receive this message, please contact the Webmaster. 
	<!-- end includes/browserdetect.cfm -->
	</cfoutput>
	<cfabort>
</cfif>