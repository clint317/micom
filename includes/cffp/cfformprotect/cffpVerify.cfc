...
">
	...
	<cffunction name="init" access="public" output="false" returntype="cffpVerify">
		<cfargument name="ConfigPath" required="false" default="/includes/cffp/cfformprotect" type="string" />
		<cfscript>
		...
		if (TotalPoints GTE getConfig().failureLimit)	{
			...
			try	{
				if (getConfig().logFailedTests)	{
					if (NOT Len(getConfig().logFile))	{ variables.Config.logFile = "CFFormProtect"; }
	...
	<cffunction name="testAkismet" access="private" output="false" returntype="struct"
		...
		<cfelse>
			<cflog text="Akismet API Key is invalid" />
		</cfif>
		<cfreturn Result />
	</cffunction>
			...
			<cfoutput>
				...
				<ol>
					...
					<cfif StructKeyExists(arguments.TestResults,"timedFormSubmission") AND NOT arguments.TestResults.timedFormSubmission.Pass>
						<cfif isDefined("formTimeElapsed")>
						<li>The time it took to fill out the form was 
							<cfif formTimeElapsed lt getConfig().timedFormMinSeconds>
								too short.
							<cfelseif formTimeElapsed gt getConfig().timedFormMaxSeconds>
								too long.
							</cfif>
					...
					<cfelseif getConfig().akismet AND NOT ListFind(arguments.testsThatFailed,"akismet")>
					...
					<cfif StructKeyExists(arguments.TestResults,"TooManyUrls")>
	...
	<cffunction name="logFailure" acces="private" output="false" returntype="void">
		...
		<cfif StructKeyExists(arguments.TestResults,"timedFormSubmission") AND NOT arguments.TestResults.timedFormSubmission.Pass>
			<cfif isDefined("formTimeElapsed")>
				<cfset LogText = LogText & "--- The time it took to fill out the form was " />
				<cfif formTimeElapsed lt getConfig().timedFormMinSeconds>
					<cfset LogText = LogText & "too short." />
				<cfelseif formTimeElapsed gt getConfig().timedFormMaxSeconds>
					<cfset LogText = LogText & "too long." />
				</cfif>
				
				<cfset LogText = LogText & "It took them #formTimeElapsed# seconds to submit the form, and your allowed	threshold 
						is #getConfig().timedFormMinSeconds#-#getConfig().timedFormMaxSeconds# seconds." />
			<cfelse>
				<cfset LogText = LogText & "The time it took to fill out the form did not fall within your
								configured threshold of #getConfig().timedFormMinSeconds#-#getConfig().timedFormMaxSeconds#
								seconds.  Also, I think the form data for this field was tampered with by the
								spammer." />
			</cfif>
		...
		<cfif StructKeyExists(arguments.TestResults,"hiddenFormField") AND NOT arguments.TestResults.hiddenFormField.Pass>
			<cfset LogText = LogText & "The hidden form field that is supposed to be blank contained data.</li>" />
		</cfif>
		...
		<cfif StructKeyExists(TestResults,"TooManyUrls")>
