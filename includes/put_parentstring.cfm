...
<cfelseif isDefined("Request.GetRelatedCats.recordcount") and isNumeric(attributes.ParentCat) and not invalid>
	...
	<cfquery name="CatQuery" dbtype="query">
		...
		WHERE Category_ID = #Val(attributes.ParentCat)#
	</cfquery>
</cfif>
...
<!--- Shopping Cart --->
	...
	<cfdefaultcase>
		<cfif Session.User_ID IS 0>			
			...
			<div id="menu_trail">Welcome! <a href="#XHTMLFormat('#Request.SecureSelf#?fuseaction=users.manager#Request.AddToken#&xfa_login_successful=#urlEncodedFormat(attributes.xfa_success)#')#" #doMouseover('Sign In')#>Please sign in or register here</a></div>
		<cfelseif fusebox.fuseaction IS "admin">
       		<div id="menu_trail"><a href="#XHTMLFormat('#Request.SecureSelf#?fuseaction=home.admin#request.token2#')#" #doMouseover('Admin Menu')#>Admin Menu</a></div> 
		<cfelse>
