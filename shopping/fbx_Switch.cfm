...
<!--- This fuseaction is used for external links to the shopping function, to make sure cookies get properly set --->
<cfelseif CompareNoCase(fusebox.fuseaction, "sendorder") IS 0 AND isDefined("attributes.Product_ID") AND isNumeric(attributes.Product_ID)>
	
    <cfoutput>
    <script type="text/javascript">
        window.location.href = '#self#?fuseaction=shopping.order&Product_ID=#attributes.Product_ID#&refresh=yes#Request.Token2#';
    </script>
	</cfoutput>
...
<cfelseif CompareNoCase(fusebox.fuseaction, "basket") IS 0>
	...
	<cfelseif isdefined("attributes.Checkout") OR isdefined("attributes.Checkout.x")>
		<cfinclude template="basket/act_recalc.cfm">
		<cflocation url="#Request.SecureSelf#?fuseaction=shopping.checkout#Request.AddToken#" addtoken="No">
	<cfelse>
