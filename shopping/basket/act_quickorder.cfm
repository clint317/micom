...
<cfloop index="num" from="1" to="#attributes.product_count#">
	...
	<cfif len(trim(attributes['item' & num])) AND isNumeric(attributes['quant' & num])>
		...
		<cfquery name="Product" datasource="#Request.DS#" username="#Request.DSuser#" password="#Request.DSpass#" maxrows="1">
			SELECT P.Product_ID 
			...
			AND P.Display = 1
			AND P.NotSold = 0
			AND P.AccessKey IN (<cfqueryparam cfsqltype="cf_sql_integer" value="#accesskeys#" list="true">)
