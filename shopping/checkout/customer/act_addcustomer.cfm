...
<cfelse><!--- this record does not exist. Is it an update or brand new? --->
	...
	<cfelse><!--- Temp was a copy that has now changed. Check if this ID was used as customer or shipping in any orders. ---->
		...
		<cfquery name="FindOrdersCustomer" datasource="#Request.DS#" username="#Request.DSuser#" password="#Request.DSpass#" maxrows="1">
			...
			WHERE C.Customer_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#CopiedFrom#">
		</cfquery>
	
		<cfquery name="FindOrdersShipto" datasource="#Request.DS#" username="#Request.DSuser#" password="#Request.DSpass#" maxrows="1">
			SELECT C.Customer_ID, N.Order_No
			FROM #Request.DB_Prefix#Order_No N 
...
<cfif Session.User_ID and UpdateUser AND GetTempRecord.Company IS NOT "PayPal Account">
	<cfquery name="UpdateUser" datasource="#Request.DS#" username="#Request.DSuser#" password="#Request.DSpass#">
		...
		SET #this_field# = <cfqueryparam cfsqltype="cf_sql_integer" value="#This_ID#">
