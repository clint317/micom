...
//Determine which payment options to display
	if (total is 0 AND NOT Trial) 
		...
		show_ppexpress = 1;
	else if (get_Order_Settings.UsePayPal AND get_Order_Settings.PayPalMethod IS "Express" AND isDefined("Session.PP_Token")) 
		show_ppexpress = 1;
	else if (get_Order_Settings.UseCRESecure)
		show_cresecure = 1;
	...
	numtypes = noPayment + show_cc + show_billcard + show_paypal + show_ppexpress + show_cresecure + show_offline + show_PO;
	typecount = 0;
</cfscript>
</cfsilent>

<!--- if using CRESecure, and no other payment options available, jump directly to the form page --->
<!--- <cfif show_cresecure AND numtypes IS 1>
	<cfinclude template="cresecure/act_send_cresecure.cfm">
</cfif> --->

<!-- start shopping/checkout/dsp_payment_options.cfm -->
<div id="dsppaymentoptions" class="shopping">
...
<cfif show_cresecure>
	...
	<cfoutput><td valign="top" width="#iif(numtypes IS 1, DE('100%'), DE('50%'))#" align="center"></cfoutput>
	<cfmodule template="../../customtags/format_input_form.cfm"
	box_title="Pay For Your Order"
	width="#iif(numtypes IS 1, DE('50%'), DE('100%'))#"
	required_fields = "0"
	>
	<cfinclude template="cresecure/put_cresecure_form.cfm">
	</cfmodule>	
	</td>
	<cfif typecount MOD 2 IS 0></tr></cfif>
</cfif> 
...
<cfif show_paypal>
	...
	required_fields = "0"
	>
		<!--- Use full shopping cart PayPal module --->
...
<cfif show_cc>
	...
	<cfquery name="GetCards" datasource="#Request.DS#" username="#Request.DSuser#" 
	...
	<cfset variables.postURL="#request.self#?fuseaction=shopping.checkout&step=payment#request.token2#">
	<cfif get_Order_Settings.CCProcess IS "Shift4OTN">
		...
		<cfif get_Shift4OTN_Settings.i4GoURL NEQ "">
			...
			<cfset variables.postURL="https://#get_Shift4OTN_Settings.i4GoURL#">
			<cfset request.offsitePostParams=structNew()>
...
<!-- end shopping/checkout/dsp_payment_options.cfm -->
