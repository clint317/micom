...
<!--- If this is a return from Shift4 i4Go, normalize the token data information --->
<cfif attributes.step IS "shift4_i4go">
	<cfinclude template="creditcards/shift4/act_shift4i4go.cfm">
</cfif>
...
<cfif (isDefined("attributes.login") OR isDefined("attributes.submit_login")) AND NOT GetCustomer.RecordCount>
...
<!--- Make sure user is not trying to skip terms step --->	
<cfelseif ListFind("shipping,payment,receipt", attributes.step) AND len(get_Order_Settings.AgreeTerms) AND NOT StructKeyExists(Session, "AgreeTerms")>
	<cfset attributes.step ="terms">
	
<!--- Make sure user is not trying to skip registration step --->	
<cfelseif ListFind("shipping,payment,receipt", attributes.step) AND ( get_Order_Settings.NoGuests OR Session.CheckoutVars.Download OR Session.CheckoutVars.Membership ) AND Session.User_ID IS 0 >
...
<cfelse>
	...
	<cfif len(get_Order_Settings.mintotal) AND Session.CheckoutVars.TotalBasket LT get_Order_Settings.mintotal AND attributes.step is not "ipn">
		<cfset attributes.step = "error">
...
<cfswitch expression = "#attributes.step#">
		...
		<cfif isdefined("attributes.SubmitAddress")>
			...
			AND len(trim(attributes.Zip)) GTE 3 AND len(trim(attributes.Phone)) AND NOT emailerror>	
				...
				<cfelse>
					...
					<cfset ShipTypes = ValueList(ShipSettings.ShipType )>					
					<cfset addressOK="yes">
					
					<!--- If using address verification, run now --->
					<cfif ListFind( ShipTypes, "UPS" ) AND NOT isDefined("attributes.addressverify")>					
						<cfinclude template="customer/act_ups_address_verify.cfm">		
					<cfelseif ListFind( ShipTypes, "USPS" ) AND NOT isDefined("attributes.addressverify")>					
	...
	<cfcase value="payment">
		...
		<!--- Check if this is a CRE Secure template request ----->
		<cfif isdefined("attributes.show") AND attributes.show is "template">
			<cfset attributes.step = "payment">
			<cfinclude template="customer/qry_get_tempshipto.cfm">
			
		<!--- Process form ----->
		<cfelseif not isdefined("attributes.backorder") AND isdefined("attributes.SubmitPayment")>
			<cfif isDefined("Session.FinalCheckoutStep")>
				<cfset attributes.step = "error">
				<cfset message = "Please do not attempt to complete the checkout more than once, or a duplicate order may be submitted. <br/>If you think this message was received in error, return to the shopping cart and restart the checkout process.">
			<cfelseif isDefined("attributes.Offline") and attributes.Offline IS NOT "BillUser">
	...
	<cfcase value="cresecure">
	<!--- return from CRE Secure received --->
		<cfinclude template="cresecure/act_cre_process.cfm">
	</cfcase>
...
<cfswitch expression = "#attributes.step#">
	...
	<cfcase value="payment">
		...
		<!--- Check if this is a request from CRE Secure for the payment template --->
		<cfif StructKeyExists( attributes, "show" ) AND attributes.show IS "template">
			<cfinclude template="dsp_invoice_header.cfm">
			<cfinclude template="cresecure/dsp_template.cfm">
		
		<!--- Show Payment options the first time through --->
		<cfelseif not isdefined("attributes.submitpayment")>		
			<cfinclude template="dsp_invoice_header.cfm">
