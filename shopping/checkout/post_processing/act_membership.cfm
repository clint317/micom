...
<cfquery name="GetMembershipInfo"  datasource="#Request.DS#" username="#Request.DSuser#" password="#Request.DSpass#" Maxrows="1">
...
<cfquery name="GetOldMembership" datasource="#Request.DS#" username="#Request.DSuser#" password="#Request.DSpass#" Maxrows="1">
...
<cfif GetMembershipInfo.Prod_Type is "membership" AND GetMembershipInfo.recur_product_ID GT 0 AND GetOldMembership.recordcount>
	...
	<cfquery name="UpdateFraudUser" datasource="#Request.DS#" username="#Request.DSuser#" password="#Request.DSpass#">
		UPDATE #Request.DB_Prefix#Users 
		SET AdminNotes = <cfqueryparam cfsqltype="cf_sql_longvarchar" value="#DateFormat(Now(),"mm/dd/yy")#: Attempted repurchase of trial Membership ID #GetOldMembership.membership_ID#. <br/>#qry_get_user.AdminNotes#">
		WHERE User_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#User_ID#">
	</cfquery>		
	
	<cfquery name="GetComments" datasource="#Request.DS#" username="#Request.DSuser#" password="#Request.DSpass#">
...
<cfelse>
	...
	<cfif GetMembershipInfo.prod_type is "membership" AND GetMembershipInfo.recur_product_ID GT 0 AND isDefined("crypt.value")>
		...
		<cfquery name="CheckCCMembership" datasource="#Request.DS#" username="#Request.DSuser#" password="#Request.DSpass#" maxrows="1">
			...
			WHERE M.Product_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#qry_get_basket.Product_ID#">
		...
		<cfif checkCCMembership.recordcount>
			...
			<cfquery name="UpdateFraudUser" datasource="#Request.DS#" username="#Request.DSuser#" password="#Request.DSpass#">
			UPDATE #Request.DB_Prefix#Order_No 
			SET Comments = <cfqueryparam cfsqltype="cf_sql_longvarchar" value="The credit card on this order has already been used previously to purchase a trial membership. Your membership will be inactive until reviewed by the store administrator.<br/>#GetComments.Comments#">
			WHERE Order_No = <cfqueryparam cfsqltype="cf_sql_integer" value="#New_OrderNo#">
			</cfquery>	
	...
	<cfif GetOldMembership.recordcount AND isDate(GetOldMembership.expire)>
		...
		<cfquery name="TurnOffRenews" datasource="#Request.DS#" username="#Request.DSuser#" password="#Request.DSpass#">
	...
	<cfif offlinePayment is "Online" AND not len(attributes.Offline) AND NOT isDefined("PayPal") AND GetMembershipInfo.Recur>
		...
		<cfif get_Order_Settings.CCProcess IS "Shift4OTN">			
			...
			<!---- Get User CC Info ----->
			<cfquery name="GetUserCard" datasource="#Request.DS#" username="#Request.DSuser#" password="#Request.DSpass#" maxrows=1>
			...
			<cfif NOT len(GetUserCard.EncryptedCard) OR GetUserCard.CardisValid is 0 OR Not isDate(GetUserCard.CardExpire) OR DateCompare(GetUserCard.CardExpire,now(),'m') lt 1>
				...
				<cfquery name="UpdateCardInfo" datasource="#Request.DS#" username="#Request.DSuser#" password="#Request.DSpass#">
					UPDATE #Request.DB_Prefix#Users 
					SET 
