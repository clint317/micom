...
<cfquery name="GetSettings" datasource="#Request.DS#" username="#Request.DSuser#" password="#Request.DSpass#" maxrows="1">
	SELECT Transtype, Username, Password, Setting1 
...
<cfset amount = GetTotals.OrderTotal * 100>
...
<cfif result is "ERROR">
  <cfset ErrorMessage = Request.objRB.getKey('cc.NoResponse') & " " & Request.objRB.getKey('cc.WaitAwhile')>
  <cfset display = "Yes">
...
<!--- If FAILED ---->
<cfelse>
  <cfset ErrorMessage =  "#Request.objRB.getKey('cc.NoVerify')# - " & ListGetAt(Result,"2")>
  <cfset display = "Yes">
