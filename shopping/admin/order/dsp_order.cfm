...
<cfset session.formKeys.orderBilling = formkey>
<cfset session.CertCodes.orderBilling = formkey>
...
style="color:###Request.GetColors.InputTText#">
	...
	<tr height="30">
		...
		<!--- return to listing --->
		<td width="53%"><img src="#Request.ImagePath#icons/lleft.gif" border="0" valign="middle" alt="" hspace="2" vspace="0" /> <a href="#session.Page#">return to list</a>
		</td>
	...
	<tr class="formbutton">
		...
		<button class="formbutton" onclick="JavaScript:newWindow=window.open( '#self#?fuseaction=shopping.admin&order=print&print=invoice&Order_No=#attributes.order_no#&redirect=1#Request.Token2#', 'Order', 'width=900,height=800,toolbar=1,location=0,directories=0,status=0,menuBar=1,scrollBars=1,resizable=1' ); newWindow.focus()">Print Invoice</button> 
		</cfif> 
		</td>
		</td>
	</tr>
	<tr><td colspan="2"><img src="#Request.ImagePath#spacer.gif" height="3" alt="" /></td></tr>
	...
	<tr>
		...
		<cfif len(GetOrder.Comments)>#HTMLEditFormat(GetOrder.Comments)#<cfelse>None</cfif><br/>
		<cfif len(GetOrder.Giftcard)>
			<strong>Giftcard:</strong>#HTMLEditFormat(GetOrder.Giftcard)#<br/>
		</cfif>		
		<cfif len(GetOrder.Delivery)>
			<strong>Ship to arrive by:</strong> #HTMLEditFormat(GetOrder.Delivery)#<br/>
		</cfif>
		<cfif len(GetOrder.Coup_Code)>
			<strong>Coupon:</strong> <cfif len(GetOrder.Coup_Code)>#GetOrder.Coup_Code#<cfelse>#ValueList(GetOrder.Disc_Code)#</cfif><br/>
		</cfif>
		...
		<cfloop index="x" from="1" to="3">
			<cfif len(GetOrder['CustomText' & x][1])>
			<strong>#get_Order_Settings['CustomText' & x][1]#: </strong> 
			#HTMLEditFormat(GetOrder['CustomText' & x][1])#<br/><br/>
			</cfif>
		</cfloop>
		<cfloop index="x" from="1" to="2">
			<cfif len(GetOrder['CustomSelect' & x][1])>
			<strong>#get_Order_Settings['CustomSelect' & x][1]#: </strong> 
			#HTMLEditFormat(GetOrder['CustomSelect' & x][1])#<br/><br/>
			</cfif>
		</cfloop>
		</td>
	</tr>
	...
	<tr class="formbutton">
		<td colspan="2"  align="right">
		<cfif not isdefined("attributes.edit")>
			<cfif len(getorder.termsused)>
				<button class="formbutton" onclick="JavaScript: newWindow = openWin( '#self#?fuseaction=shopping.admin&order=ViewTerms&order_no=#attributes.order_no##request.token2#', 'Terms', 'width=650,height=700,toolbar=0,location=0,directories=0,status=1,menuBar=0,scrollBars=1,resizable=1' ); newWindow.focus()">View Terms</button>
			</cfif>
			<cfmodule template="../../../access/secure.cfm"
			keyname="shopping"
			requiredPermission="64"
			>
			<cfif GetOrder.paid IS 1 AND len(GetOrder.TransactNum)>
				<button class="formbutton" onclick="Javascript:location.href='#self#?fuseaction=shopping.admin&order=display&order_no=#attributes.order_no#&edit=refund&redirect=1#request.token2#';">Send Refund</button>
			</cfif>
...
<cfelseif isdefined("attributes.edit") and attributes.edit is "refund">
	<tr>
		<td colspan="2">
			<cfinclude template="put_refund.cfm">
		</td>
	</tr>
   ...
   <strong>Invoice:</strong> ###GetOrder.InvoiceNum#<br/></cfif>		
			...
			<cfelse>
				...
				<cfif GetOrder.AuthNumber IS NOT "0" AND len(GetOrder.AuthNumber)>
					...
					requiredPermission="64"
						>
						<br/><a href="#request.self#?fuseaction=shopping.admin&order=display&order_no=#attributes.order_no#&edit=pay&act=charge&formkey=#Hash(formkey,"SHA-256")##request.token2#"><span style="color: red; font-weight: bold;">Capture Funds</span></a></cfmodule>
					</cfif>
				</cfif>
				</cfloop>
		...
		<cfif GetCerts.RecordCount>
			...
			<cfelse>[<a href="#self#?fuseaction=shopping.admin&order=display&order_no=#attributes.order_no#&sendcodes=yes&formkey=#Hash(formkey,"SHA-256")##request.token2#">send codes</a>]
			</cfif><br/>
		</cfif>
		
		<cfif len(getorder.admin_updated)>
			<br/><strong>Last update:</strong>  <cfif len(GetOrder.Admin_Updated)>#LSDateFormat(GetOrder.Admin_Updated, "m/d/yy")#</cfif> by #GetOrder.Admin_Name#<br/>
		</cfif>
		</td>
	</tr>
...
<cfelse>
	...
	<tr><td colspan="2"><img src="#Request.ImagePath#spacer.gif" height="4" alt="" /></td></tr>
	<tr class="formbutton">
		<td colspan="2"  align="right">
		<cfif not isdefined("attributes.edit")>
	...
	requiredPermission="64"
	>			
		<button class="formbutton" onclick="JavaScript: newWindow = openWin( '#self#?fuseaction=shopping.admin&order=AddProduct&order_no=#attributes.order_no##request.token2#', 'Products', 'width=650,height=600,toolbar=0,location=0,directories=0,status=1,menuBar=0,scrollBars=1,resizable=1' ); newWindow.focus()">Add Product</button>
	...
	<tr class="formbutton">
		...
		<button class="formbutton" onclick="Javascript:newWindow=window.open( '#self#?fuseaction=shopping.admin&order=print&print=packlist&Order_No=#attributes.order_no#&redirect=1#Request.Token2#', 'PO', 'width=900,height=800,toolbar=1,location=0,directories=0,status=0,menuBar=1,scrollBars=1,resizable=1' ); newWindow.focus()">Print Packing Slip</button>  
