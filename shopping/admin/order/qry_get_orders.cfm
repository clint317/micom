...
<cfquery name="GetMinDate" datasource="#Request.DS#" username="#Request.DSuser#" password="#Request.DSpass#">
...
<cfset attributes.sortorder = Trim(sanitize(attributes.sortorder))>
...
<cfquery name="qry_get_Orders" datasource="#Request.DS#" username="#Request.DSuser#" password="#Request.DSpass#">
	...
	<cfif len(attributes.prodid) AND isNumeric(attributes.prodid)>
		AND I.Product_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#attributes.prodid#">
	</cfif>
	
	<cfif len(attributes.Order_No)>
		AND O.Order_No = <cfqueryparam cfsqltype="cf_sql_integer" value="#(attributes.Order_No - Get_Order_Settings.BaseOrderNum)#">
	</cfif>
	
	 <cfif len(attributes.offlinepayment)>
		 AND O.OfflinePayment = <cfqueryparam cfsqltype="cf_sql_varchar" value="#offlinepayment#">
 	</cfif> 
	
	<cfif len(attributes.UID)>
		AND O.User_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#attributes.UID#">
	</cfif>
	
	<cfif len(attributes.customer)>
		AND (FirstName Like <cfqueryparam cfsqltype="cf_sql_varchar" value="%#attributes.customer#%">
		OR LastName Like <cfqueryparam cfsqltype="cf_sql_varchar" value="%#attributes.customer#%">
		OR Company Like <cfqueryparam cfsqltype="cf_sql_varchar" value="%#attributes.customer#%">) 
	</cfif>
	
	<cfif len(attributes.filled)>
		AND Filled = <cfqueryparam cfsqltype="#Request.SQL_Bit#" value="#attributes.filled#">
	</cfif>

	<cfif len(attributes.paid)>
		AND Paid = <cfqueryparam cfsqltype="#Request.SQL_Bit#" value="#attributes.paid#">
	</cfif>
	
	<cfif len(attributes.affiliate)>
  		AND Affiliate = <cfqueryparam cfsqltype="cf_sql_integer" value="#attributes.affiliate#">
 	</cfif>
	
	<cfif isDefined("filledStart")>
		AND DateFilled >= <cfqueryparam cfsqltype="cf_sql_timestamp" value="#filledStart#">
		AND DateFilled <= <cfqueryparam cfsqltype="cf_sql_timestamp" value="#filledTo#">
	</cfif>
		
	<cfif len(attributes.StartDate) AND filledsearch >
		AND DateOrdered >= <cfqueryparam cfsqltype="cf_sql_timestamp" value="#attributes.StartDate#">
	</cfif>
	<cfif len(attributes.ToDate) AND filledsearch>
		AND DateOrdered <= <cfqueryparam cfsqltype="cf_sql_timestamp" value="#attributes.ToDate#">
	</cfif>

	<cfif isDefined("orderfrom") AND NOT isDefined("attributes.show")>
		AND DateOrdered >= <cfqueryparam cfsqltype="cf_sql_timestamp" value="#orderfrom#">
		AND DateOrdered <= <cfqueryparam cfsqltype="cf_sql_timestamp" value="#orderto#">
	</cfif>
...
<!-- Convert dates for paging --->
<cfset dateList = "startdate,todate,orderfrom,orderto,filledstart">
<cfloop list="#dateList#" index="thedate">
	<cfif StructKeyExists( attributes, thedate ) >
		<cfset attributes[thedate] = DateFormat(attributes[thedate], 'mm-dd-yyyy') >
	</cfif>
</cfloop>
