...
<cfquery name="GetBasket" datasource="#Request.DS#" username="#Request.DSuser#" password="#Request.DSpass#">
	SELECT O.*, P.Name 
...
<cfquery name="GetCustomer" datasource="#Request.DS#" username="#Request.DSuser#" password="#Request.DSpass#" maxrows="1">
...
<cfquery name="GetShipTo" datasource="#Request.DS#" username="#Request.DSuser#" password="#Request.DSpass#" maxrows="1">
	SELECT * FROM #Request.DB_Prefix#Customers 
	WHERE Customer_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#GetOrder.ShipTo#">
</cfquery>
...
<cfif NOT GetShipTo.RecordCount>
	<cfset GetShipTo = duplicate(GetCustomer)>
</cfif>
...
<cfloop index="attributes.account_ID" list="#basket_dropshipper_list#">
	...
	<cfquery name="Check_Account" datasource="#Request.DS#" username="#Request.DSuser#" password="#Request.DSpass#" maxrows="1">
		SELECT Dropship_Email, Acc_PO_text FROM #Request.DB_Prefix#Account
		WHERE Account_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#attributes.account_ID#">
	</cfquery>
	...
	<cfif len(Check_Account.dropship_email) AND EmailDropShip>
		...
		<cfif len(Check_Account.Acc_PO_text)>
			<cfset Message = Message & "Special Instructions: " & Check_Account.Acc_PO_text & LineBreak>
		</cfif>
...
Customer Order Number #(attributes.Order_No + get_order_settings.BaseOrderNum)#
-------------------------
#BillString#
