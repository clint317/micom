...
<cfset fieldlist="order,order_no,customer,orderdate,startdate,todate,filled,paid,affiliate,status,sort,sortorder">
...
<table width="100%" cellspacing="4" border="0" cellpadding="0" class="formtext" style="color:###Request.GetColors.InputTText#">	
	...
	<tr>
		...
		<option value="ordernum" #doSelected(attributes.sort,'ordernum')#>Order No</option>
	...
	<!--- Extra query needed due to Access not supporting "Distinct" for aggregate functions --->
		<cfquery name="getProds" datasource="#Request.DS#" username="#Request.DSuser#" password="#Request.DSpass#">
			...
			FROM (SELECT DISTINCT Product_ID, SUM(Quantity) AS Items FROM #Request.DB_Prefix#Order_Items
					WHERE Order_No = <cfqueryparam cfsqltype="cf_sql_integer" value="#qry_get_Orders.Order_No#">
