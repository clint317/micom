...
<cfset fieldlist="order,order_no,customer,datefilled,orderdate,startdate,todate,filled,paid,affiliate,status,sort,sortorder,show">
...
<cfif qry_get_Orders.recordcount gt 0>					
	...
	<cfloop query="qry_get_Orders" startrow="#PT_StartRow#" endrow="#PT_Endrow#">
		...
		<td><cfif edituser><cfif uid><a href="#self#?fuseaction=users.admin&user=summary&newWindow=yes&uid=#uid##Request.Token2#" target="user"><cfelse><a href="##" onclick="Guest();return false;"></cfif>#firstname#  #lastname#</a>	
	...
	<tr>
		<td colspan="4" height="25" align="center" class="formtext">
			...
			<input type="submit" name="purge" value="clear month" class="formbutton" onclick="return confirm('Are you sure you want to do this? \n Clearing will permanently remove the orders from the database.');" />
		</td>
		<td colspan="3" align="right" class="formtext">
			<input type="submit" name="purge" value="delete checked" class="formbutton" onclick="return confirm('Are you sure you want to do this? \n Deleting will permanently remove the order(s) from the database.');" />&nbsp;
			<cfset fieldlist="order,StartMonth,StartDay,StartYear,ToMonth,ToDay,ToYear,uid,datefilled,status,order_no,customer,affiliate,sort,sortorder">
			<cfinclude template="../../../includes/act_setpathvars.cfm">
