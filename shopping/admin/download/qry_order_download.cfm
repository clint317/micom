...
<cftry>
	...
	<cfif len(attributes.FromDate)>
		...
		<cfelseif  listlen(attributes.FromDate,'-') is 3>
			<cfset FromDate = attributes.FromDate>
		</cfif>
	</cfif>
	
	<cfif len(attributes.ToDate)>
		<cfif listlen(attributes.ToDate,'-') is 1>
			<cfset ToDate = CreateDateTime(#attributes.ToDate#, 12, 31, 23, 59, 59)>
...
C.Residence as Billto_Resident,
...
S.Residence as Shipto_Resident,
D.*,
...
WHERE 1=1
	<cfif isNumeric(attributes.FromOrder)>
		AND O.Order_No >= <cfqueryparam cfsqltype="cf_sql_integer" value="#attributes.FromOrder-Get_Order_Settings.BaseOrderNum#">
	</cfif>	
	<cfif isNumeric(attributes.ToOrder)>
		AND O.Order_No <= <cfqueryparam cfsqltype="cf_sql_integer" value="#attributes.ToOrder-Get_Order_Settings.BaseOrderNum#">
	</cfif>	
	<cfif isDate(attributes.FromDate)>
		AND N.DateOrdered >= <cfqueryparam cfsqltype="cf_sql_timestamp" value="#FromDate#"> 
	</cfif>	
