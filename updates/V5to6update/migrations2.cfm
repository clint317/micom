...
<cfquery name="MoveOptChoice" datasource="#Request.DS#" username="#Request.DSuser#" password="#Request.DSpass#">
UPDATE Order_Items 
SET OptChoice = OptQuant
</cfquery>

<cfquery name="UpdOptQuant" datasource="#Request.DS#" username="#Request.DSuser#" password="#Request.DSpass#">
UPDATE Order_Items 
...
<cfquery name="GetStdOptions" datasource="#Request.DS#" username="#Request.DSuser#" password="#Request.DSpass#">
...
<cfloop query="GetStdOptions">
	...
	<cfloop index="num" from="1" to="#NumChoices#">
		<cfquery name="AddChoice" datasource="#Request.DS#" username="#Request.DSuser#" password="#Request.DSpass#">
...
<cfloop query="GetOptions">
	...
	<cfif NumChoices GT 0>
		<cfloop index="num" from="1" to="#NumChoices#">
			<cfif ListLen(SKUs, "^") IS NumChoices>
				<cfset SKU = ListGetAt(SKUs, num, "^")>
			<cfelse>
				<cfset SKU = 0>
			</cfif>
			<cfif ListLen(Quants, "^") IS NumChoices>
				<cfset NuminStock = ListGetAt(Quants, num, "^")>
				<!--- If quantity found on any of these, use this for an inventory tracking option --->
			...
			<cfif ListLen(DisplayList, "^") IS NumChoices>
				<cfset DisplayThis = ListGetAt(DisplayList, num, "^")>
			<cfelse>
				<cfset DisplayThis = "">
			</cfif>
			...
			#iif(isNumeric(DisplayThis), DisplayThis, 1)#, #num#)
			</cfquery>	
		</cfloop>
	...
	<cfelseif ListGetAt(SKUs, 1, "^") IS NOT 0 OR ListLen(Quants, "^")>
		<cfloop index="num" from="1" to="#NumSKUs#">
			<cfif ListLen(SKUs, "^") IS NumSKUs>
				<cfset SKU = ListGetAt(SKUs, num, "^")>
			<cfelse>
				<cfset SKU = 0>
			</cfif>
			<cfif ListLen(Quants, "^")IS NumSKUs>
				<cfset NuminStock = ListGetAt(Quants, num, "^")>
				<!--- If quantity found on any of these, use this for an inventory tracking option --->
				<cfif NuminStock IS NOT 0>
					<cfset OptQuant = new_id>
			...
			<cfif ListLen(DisplayList, "^")IS NumSKUs>
				<cfset DisplayThis = ListGetAt(DisplayList, num, "^")>
			<cfelse>
				<cfset DisplayThis = "">
			</cfif>
	...
	<cfif OptQuant IS NOT 0>
		<cfquery name="UpdProducts" datasource="#Request.DS#" username="#Request.DSuser#" password="#Request.DSpass#">
			UPDATE Products
...
<cfquery name="RemoveProducts" datasource="#Request.DS#" username="#Request.DSuser#" password="#Request.DSpass#">
